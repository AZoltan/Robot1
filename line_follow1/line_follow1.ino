...
void loop() {
    ...
    if (line[2] > (avg + 20) && line[3] > (avg + 20) && line[4] > (avg + 20)) {
        motor_stop();
    } else if (line[2] > (avg + 20) && line[3] > (avg + 20)) {
        motor_set_direction(BACKWARD, FORWARD);
        motor_set_speed(MAX_SPEED, MAX_SPEED);
    } else if (line[3] > (avg + 20) && line[4] > (avg + 20)) {
        motor_set_direction(FORWARD, BACKWARD);
        motor_set_speed(MAX_SPEED, MAX_SPEED);    
    } else if (line[3] > (avg + 20)) {
        // go forward
        motor_go_forward();
    } else {
        if (line[2] > (line[4] + 20)) {
